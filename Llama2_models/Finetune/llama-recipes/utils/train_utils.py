...
def distill_loss(student_logits, teacher_logits, temperature=1.0, alpha=1.0):
    """
    Calculates the distillation loss using the student and teacher logits
    
    Args:
        student_logits: The logits from the student model, shape (B, L, N)
        teacher_logits: The logits from the teacher model, shape (B, L, N)
        temperature: The temperature to use for the distillation loss
        alpha: The weight to use for the distillation loss
        
    Returns: The distillation loss on the last dimension
    """
    ...
    # Scale the loss by the alpha parameter
    loss = loss * (alpha * temperature ** 2)
    return loss 
...
def train_distill(student_model, teacher_model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None):
    """
    Trains the model on the given dataloader
    
    Args:
        student_model: The model to be trained
        teacher_model: The teacher model to be used for distillation
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                # apply loss mask to remove padding tokens in dim 1
                loss = loss[loss_mask]
                loss= loss.sum() / loss_mask.sum()
                mlm_loss = student_outputs.loss
                loss = train_config.coef_dist * loss + train_config.coef_mlm * mlm_loss
        ...
        if train_config.run_validation:
            eval_ppl, eval_epoch_loss = evaluation(student_model, train_config, eval_dataloader, local_rank, tokenizer)
            ...
            if train_config.save_model and eval_epoch_loss < best_val_loss:
                ...
                if train_config.use_peft:
                    if train_config.enable_fsdp:
                        ...
                        print(f"we are about to save the PEFT modules")
                    student_model.save_pretrained(os.path.join(train_config.output_dir, str(epoch)))
                    if train_config.enable_fsdp:
                        if rank==0: 
                    ...
                    if not train_config.use_peft and not train_config.enable_fsdp:
                        ...
                        # save model
                        torch.save(student_model.state_dict(), save_full_path)
    
                    elif not train_config.use_peft and fsdp_config.checkpoint_type == StateDictType.FULL_STATE_DICT:
                        
                        model_checkpointing.save_model_checkpoint(
                            student_model, optimizer, rank, train_config, epoch=epoch
                        )
            ...
            if train_config.save_model:
                ...
                if train_config.use_peft:
                    ...
                    if not train_config.use_peft and fsdp_config.checkpoint_type == StateDictType.FULL_STATE_DICT:
                        ...
                        model_checkpointing.save_model_and_optimizer_sharded(student_model, rank, train_config)
                        if train_config.save_optimizer:
                            model_checkpointing.save_model_and_optimizer_sharded(student_model, rank, train_config, optim=optimizer)
                            print(" Saving the FSDP model checkpoints and optimizer using SHARDED_STATE_DICT")
                            print("=====================================================")
            ...
            if train_config.save_model and eval_epoch_loss < best_val_loss:
                ...
                if train_config.use_peft:
                    ...
                    if model.__class__.__name__ == "VisLlama":
                        ...
                        # create save path
                        save_dir = os.path.join(train_config.output_dir, str(epoch))
                        if not os.path.exists(save_dir):
                            os.makedirs(save_dir)
                        # save model
...
def save_non_fsdp_train_params(train_config):
    """
    This function saves the train_config and FSDP config into a train_params.yaml.
    This will be used by converter script in the inference folder to fetch the HF model name or path.
    It also would be hepful as a log for future references.
